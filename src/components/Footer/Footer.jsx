...
const Footer = () => (
  <footer className="bg-[#1a1a2e] text-gray-400 pt-[60px] pb-6 px-5">
    <div className="max-w-[1280px] mx-auto grid grid-cols-[2fr_1fr_1fr_1fr] gap-10 mb-10 max-[900px]:grid-cols-2 max-[560px]:grid-cols-1">
      ...
      <div className="flex flex-col gap-3.5">
        ...
        <div className="flex gap-2.5 mt-1">
          {[FaFacebook, FaInstagram, FaYoutube, FaTiktok].map((Icon, i) => (
            <div key={i} className="w-9 h-9 rounded-full bg-white/[0.08] flex items-center justify-center text-gray-400 text-[0.9rem] cursor-pointer border border-white/[0.08] transition-all hover:bg-primary hover:text-white hover:border-primary hover:-translate-y-0.5">
              <Icon />
            </div>
          ))}
        </div>
        <div className="flex gap-2.5 mt-3.5">
          {[{ icon: '🍎', store: 'App Store' }, { icon: '▶', store: 'Google Play' }].map(({ icon, store }) => (
            <div key={store} className="py-[7px] px-3.5 rounded-lg border border-white/[0.15] flex items-center gap-2 text-[0.75rem] text-white cursor-pointer transition-all hover:border-primary hover:bg-primary/10">
      ...
      {/* About */}
      <div>
        <div className="text-[0.85rem] font-bold text-white uppercase tracking-[0.8px] mb-4">Về SmartRent</div>
      ...
      {/* Support */}
      <div>
        <div className="text-[0.85rem] font-bold text-white uppercase tracking-[0.8px] mb-4">Hỗ trợ</div>
        <div className="flex flex-col gap-2.5">
      ...
      {/* Partner */}
      <div>
        <div className="text-[0.85rem] font-bold text-white uppercase tracking-[0.8px] mb-4">Chủ xe</div>
        <div className="flex flex-col gap-2.5">
