                ...
                <div className="flex flex-col gap-3">
                  {POPUP_OPTIONS[openPopup]?.map((option) => (
                    <button
                      key={option.value}
                      type="button"
                      className={`w-full flex items-center gap-4 rounded-[18px] border px-4 py-4 text-left transition-all ${
                    ...
                    >
                      <span className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 ${
                        tempSelection === option.value ? 'border-primary' : 'border-gray-300'
                      }`}>
                        {tempSelection === option.value && (
