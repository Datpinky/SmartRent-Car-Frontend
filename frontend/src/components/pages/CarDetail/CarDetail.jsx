...
const CarDetail = () => {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    if (!canReviewThisVehicle) {
      setReviewError('Ban chi co the danh gia sau khi hoan tat booking cho chiec xe nay.');
      return;
    }
    setReviewError('');
              ...
              {visibleGalleryImages.map((imageUrl, index) => {
                ...
                return (
                  <button
                    key={imageUrl}
                    type="button"
                    className={`rounded-xl overflow-hidden border-2 transition-all ${isActiveImage
            ...
            <div>
              <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-100">
                ...
                {canReviewThisVehicle && (
                  ...
                  >
                    {showReviewForm && !isEditingReview ? 'Hủy' : hasReviews ? '+ Thêm đánh giá' : '+ Viết đánh giá'}
                  </button>
                )}
              </div>

              {canManageReviews && reviewAccessLoading && (
                <p className="text-[0.8rem] text-gray-400 mb-3">Dang kiem tra dieu kien danh gia...</p>
              )}

              {canManageReviews && !reviewAccessLoading && !canReviewThisVehicle && (
                <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl px-4 py-3 mb-4 text-[0.8rem] leading-6">
                  Ban co the xem danh gia cua nhung renter khac tai day. Quyen viet danh gia chi mo sau khi ban hoan tat
                  it nhat 1 booking cho chiec xe nay.
                </div>
              )}
                ...
                <form onSubmit={handleReviewSubmit} className="bg-gray-50 rounded-xl p-4 mb-4 flex flex-col gap-3 border border-gray-100">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div>
                      ...
                      <div className="text-[0.76rem] text-gray-500 mt-1">
                        {isEditingReview
                    ...
                    {isEditingReview && (
                      <button
                        ...
                        className="text-[0.78rem] text-gray-500 font-semibold hover:text-primary transition-colors"
                      >
                        Hủy sửa
                      </button>
                    )}
                  ...
                  >
                    {reviewSubmitting
                      ? (isEditingReview ? 'Đang lưu...' : 'Đang gửi...')
              ...
              {reviews.map((r) => (
                <div key={r._id} className="border-b border-gray-100 py-3 last:border-0">
                  <div className="flex items-start justify-between gap-3 mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isOwnReview(r) && (
                            <span className="px-2 py-0.5 rounded-full bg-primary-light text-primary text-[0.68rem] font-semibold">
                              Đánh giá của bạn
                            </span>
                          )}
                    ...
                    {canReviewThisVehicle && isOwnReview(r) && (
                      <button
                        type="button"
                        onClick={() => openEditReviewForm(r)}
                        className="text-[0.76rem] text-primary font-semibold hover:underline shrink-0"
                      >
                        Sửa
                      </button>
                    )}
                  ...
                  <div className="ml-9">
                    {r.comment ? (
                      <p className="text-[0.82rem] text-gray-600">{r.comment}</p>
                    ) : (
                      <p className="text-[0.8rem] text-gray-400 italic">Người dùng chưa để lại nhận xét chi tiết.</p>
                    )}
                  </div>
                </div>
              ))}
              ...
              <div className="grid grid-cols-6 gap-3 max-[900px]:grid-cols-4 max-[560px]:grid-cols-3">
                {visibleGalleryImages.map((imageUrl, index) => (
                  <button
                    key={`${imageUrl}-${index}`}
                    type="button"
                    className={`rounded-xl overflow-hidden border-2 transition-all ${index === activeImageIndex ? 'border-primary' : 'border-gray-200 hover:border-primary/60'
                      }`}
                    style={{ aspectRatio: '4/3' }}
