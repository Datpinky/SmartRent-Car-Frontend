...
const Revenue = () => {
  ...
  const inputCls =
    'w-full py-2.5 px-3 border border-gray-200 rounded-lg text-[0.85rem] text-gray-900 outline-none focus:border-[#059669] focus:ring-2 focus:ring-[#059669]/15';
      ...
      <Modal
        ...
        footer={
          <>
            ...
            >
              {wLoading ? <FaSpinner style={{ animation: 'spin 0.8s linear infinite' }} /> : <FaMoneyBillWave />}
      ...
      >
        <div style={{ background: 'linear-gradient(135deg,#f0fdf4,#dcfce7)', borderRadius: 12, padding: '14px 18px', marginBottom: 20, border: '1px solid #bbf7d0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: '1.6rem', fontWeight: 800, color: '#059669', lineHeight: 1 }}>{fmtVN(stats?.pending ?? 0)}</div>
          </div>
          <FaCheckCircle style={{ fontSize: '2rem', color: '#86efac' }} />
        </div>

        <div style={{ marginBottom: 18 }}>
          <label className="block text-[0.8rem] font-semibold text-gray-700 mb-1.5">Số tiền muốn rút (đ)</label>
          <input
            className={inputCls}
            ...
            inputMode="numeric"
          />
          <div style={{ fontSize: '0.72rem', marginTop: 6, color: wAmtErr ? '#dc2626' : '#6b7280' }}>
            {wAmtErr || `Tối thiểu 100,000đ — Tối đa ${fmtVN(stats?.pending ?? 0)}`}
          </div>
        </div>

        <div style={{ marginBottom: 8 }}>
          <label className="block text-[0.8rem] font-semibold text-gray-700 mb-1.5">Tài khoản ngân hàng nhận tiền</label>
          <div style={{ position: 'relative' }}>
            <FaUniversity style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#059669', fontSize: '0.85rem', pointerEvents: 'none', zIndex: 1 }} />
            <select
              className={inputCls}
        ...
        footer={
          detailTx ? (
            <>
              ...
              >
                <FaFileContract /> Xem hợp đồng số
              </a>
              <a
              ...
              >
                <FaRobot /> Xem báo cáo hư hỏng AI
              </a>
            </>
          ) : null
        }
