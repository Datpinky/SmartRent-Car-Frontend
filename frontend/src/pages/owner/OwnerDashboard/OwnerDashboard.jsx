  ...
  return (
    <div>
      ...
      <div className="stats-grid">
        <StatCard title="Xe đang ký gửi"     value={MOCK_OWNER_VEHICLES.length}   icon={<FaCar aria-hidden="true" />}           color="#0891b2" />
        <StatCard title="Xe đang cho thuê"   value={MOCK_OWNER_VEHICLES.filter(v => v.status === 'active').length} icon={<FaCar aria-hidden="true" />} color="#00b14f" />
        <StatCard title="Tổng doanh thu"     value={(totalRevenue / 1000000).toFixed(1) + 'M'} icon={<FaMoneyBillWave aria-hidden="true" />} color="#d97706" trend={8.5} trendLabel="so tháng trước" />
        <StatCard title="Chờ nhận tiền"      value={(pendingRevenue / 1000).toLocaleString() + 'K'} icon={<FaMoneyBillWave aria-hidden="true" />} color="#7c3aed" subtext="Sẽ chuyển vào T4" />
        <StatCard title="Tổng chuyến"        value={MOCK_OWNER_VEHICLES.reduce((s, v) => s + v.trips, 0)} icon={<FaRoute aria-hidden="true" />} color="#059669" trend={14.2} trendLabel="so tháng trước" />
        <StatCard title="Doanh thu tháng 3"  value="2.2M" icon={<FaChartLine aria-hidden="true" />} color="#dc2626" />
      </div>
          ...
          {MOCK_OWNER_VEHICLES.map(v => (
            <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: '1px solid #f9fafb' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#00b14f' }} className="tabular-nums">{(v.revenue / 1000000).toFixed(1)}M</div>
