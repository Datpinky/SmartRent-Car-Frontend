...
const SECTIONS = [
  { key: 'general',      label: 'Cài đặt chung',  icon: <FaCog aria-hidden="true" /> },
  { key: 'payment',      label: 'Thanh toán',      icon: <FaMoneyBillWave aria-hidden="true" /> },
  { key: 'notification', label: 'Thông báo',       icon: <FaBell aria-hidden="true" /> },
  { key: 'security',     label: 'Bảo mật',         icon: <FaShieldAlt aria-hidden="true" /> },
];

const InputRow = ({ label, value, type = 'text', hint, onChange, name, autoComplete }) => {
  const inputId = name ? `ss-${name}` : undefined;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 16 }}>
      <label htmlFor={inputId} style={{ fontSize: '0.82rem', fontWeight: 600, color: '#374151' }}>{label}</label>
      <input
        id={inputId}
        name={name}
        type={type}
        ...
        style={{ border: '1.5px solid #e5e7eb', borderRadius: 9, padding: '9px 12px', fontSize: '0.85rem', color: '#111827' }}
      />
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #f3f4f6' }}>
      ...
      <button
        ...
        aria-checked={on}
        onClick={() => setOn(!on)}
        style={{ width: 44, height: 24, borderRadius: 50, border: 'none', background: on ? '#00b14f' : '#d1d5db', cursor: 'pointer', position: 'relative', transition: 'background-color 0.2s' }}
      >
      ...
      <div style={{ display: 'flex', gap: 20 }}>
        ...
        <div style={{ flex: 1, background: '#fff', borderRadius: 14, padding: 24, border: '1px solid #f0f0f0' }}>
          {activeSection === 'general' && <>
            ...
            <InputRow label="Tên nền tảng" name="platform-name" value="SmartRent Car" hint="Tên hiển thị trên toàn bộ hệ thống" />
            <InputRow label="Email liên hệ" name="support-email" autoComplete="email" value="support@smartrent.vn" type="email" />
            <InputRow label="Hotline hỗ trợ" name="hotline" value="1900 1234" />
            <InputRow label="Phí dịch vụ (%)" name="service-fee" value="5" type="number" hint="Phần trăm phí dịch vụ trên mỗi giao dịch" />
          </>}
          {activeSection === 'payment' && <>
            ...
            <InputRow label="Số tài khoản nhận tiền" name="bank-account" value="0123456789" />
            <InputRow label="Ngân hàng" name="bank-name" value="Vietcombank" />
          </>}
          ...
          {activeSection === 'security' && <>
            ...
            <ToggleRow label="Yêu cầu xác minh số điện thoại" checked={true} hint="Bắt buộc trước khi đặt xe" />
            <ToggleRow label="Phiên đăng nhập tự động hết hạn" checked={true} />
            <InputRow label="Thời gian phiên (phút)" name="session-timeout" value="120" type="number" />
