...
class PaymentController {
  ...
  async getPaymentIntentById(req, res, next) {
    try {
      ...
      if (!paymentIntent) {
        return res.status(404).json({ message: 'Không tìm thấy Payment Intent' });
      }
  ...
  async syncPaymentIntentWithDB(req, res, next) {
    try {
      ...
      return res.status(200).json({
        ...
        data: { intentId: intent.id, intentStatus: intent.status, paymentStatus, bookingStatus }
      });
    } catch (error) {
      next(error);
    }
